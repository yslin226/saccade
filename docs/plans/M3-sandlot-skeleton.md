...
| 取極值放大單幀偵測失敗 | **一半已修(見下),另一半 M4** |

### 取極值的問題(Task 4 實測,已修一半)

`measure()` 取整段的極值 —— 髖肩分離取最大、手肘取最小。在 `林永閎.MOV` 上原本跑出:

```
elbow_flexion_R  15.4 度   frame 17
```

人的肘關節極限約 30 度,15.4 度是解剖上不可能的。查該幀:

```
fr 17   S(408,520) E(354,605) W(413,552)   conf 1.00/0.88/0.78
```

手腕被放在折回肩膀的位置。信心 0.78 不算低 —— 這正是先前量到的
「MediaPipe 的信心對誤差是反指標(AUROC 0.358)」。

**取極值讓 23 幀裡偵測最差的那一幀直接成為指標值。**

**已修的部分:`MIN_ELBOW_DEGREES = 25.0`。** 低於此值不是「很彎的手肘」而是偵測失敗,
`elbow_valgus` 回傳 `None`,取極值自動跳過。與 `MIN_SPAN_PX` 同一類判斷 ——
某些數值範圍代表偵測器崩了,不是身體真的那樣。修後:

```
elbow_flexion_L  24.2 → 29.5 度
elbow_flexion_R  15.4 → 33.0 度        十次一致仍然成立
```

**未修的部分(M4):** 這只擋得住解剖學排除的值。手腕偏 40px 仍然算得出可能的角度,
而找出那些需要第二個偵測器來反對第一個 —— disagreement 工具(AUROC 0.638)已在
`infrastructure/saccade_tools.py` 接好但尚未接入 `measure()`。

M4 的作法:量測前先用 disagreement 標記不可信幀,取極值時排除它們。
**M4 的量化驗收應包含:有／無 disagreement 過濾的指標差異。**
