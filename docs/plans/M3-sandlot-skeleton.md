...
| **取極值放大單幀偵測失敗** | **Task 4 實測發現,M4 處理。詳見下** |

### 取極值的問題(Task 4 實測)

`measure()` 取整段的極值 —— 髖肩分離取最大、手肘取最小。在 `林永閎.MOV` 上跑出:
...
**取極值讓 23 幀裡偵測最差的那一幀直接成為指標值。** 一致性驗收仍然通過
(同樣的錯誤每次都一樣),但數字本身不可信。

不在 M3 修,理由:這正是 M4 的題目(spec 第 9 節「以主動視覺修正遮擋／模糊幀」),
而修它需要的工具 —— 兩偵測器不一致(AUROC 0.638)—— 已在
`infrastructure/saccade_tools.py` 接好但尚未接入 `measure()`。

M4 的作法:量測前先用 disagreement 標記不可信幀,取極值時排除它們。
**M4 的量化驗收應包含:排除不可信幀後,手肘角度是否落回解剖範圍。**
